...
const Header = () => {
	return (
		<header className='bg-white shadow-md'>
			<nav className='container mx-auto flex items-end justify-between py-4 px-6'>
				<Link to='/' className='flex items-end gap-2'>
					...
					<span className='text-gray-800 text-lg font-semibold leading-tight'>Budget planner manager</span>
				</Link>
				...
				<div className='flex space-x-8'>
					{['Dashboard', 'Incomes', 'Expenses'].map(item => (
						<NavLink
							...
							to={`/${item.toLowerCase()}`}
							className={({ isActive }) =>
								`text-gray-700  font-medium ${isActive ? 'underline underline-offset-4 color-dark-blue' : ''}`
							}>
							{item}
						</NavLink>
					))}
